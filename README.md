...
This work grew out of the completion geometry developed in our earlier deterministic pushdown-normalization research. The passage from the original completion-aware model to general rational residuals revealed a Diophantine completion structure, a primitive homogeneous kernel, and an associated Farey geometry. The present paper transfers that completion viewpoint to the classical Cohn–Markoff matrix setting. The main result is an exact local rigidity law for quadratic returns in the Cohn–Markoff monoid. We show that the defect created by exchanging two adjacent terminal patterns factors exactly into a positive size term and a single Farey determinant residual. This determines simultaneously the magnitude and sign of the quadratic defect and proves that equality occurs at one unique progenitor on the word orbit, without requiring the variable word itself to be palindromic. The paper also establishes an exact correspondence between the homogeneous kernel of the associated completion equation and fundamental quadratic-return translations. The novelty is therefore a new completion-geometric structure inside the classical Cohn–Markoff framework: a quadratic local defect is controlled exactly by linear Farey data, producing a rigid sign law, a unique zero, and a direct kernel-to-return correspondence.

**Exact_Unrestricted_Bounded_Replacement_State_Complexity.pdf:**  
This work grew out of our earlier completion-aware deterministic pushdown construction and its generalization to rational balance languages. That earlier line of research revealed a natural tradeoff between the amount of stack replacement allowed in one transition and the amount of finite control required. The present paper shows that this tradeoff is not an artifact of a particular stack encoding. We determine the exact state complexity for the entire unrestricted class of deterministic one-work-symbol pushdown recognizers under bounded replacement, including arbitrary history-dependent representations and deterministic epsilon normalization. We also identify the exact replacement threshold at which the optimal unbounded state complexity first becomes attainable. The main novelty is therefore a representation-independent state–replacement law: a staircase that originally appeared only in a canonical residual construction is proved to be an intrinsic and optimal descriptional-complexity property of the language family itself.

**Collapse_Retained_Information_and_Representation_Independent_Pushdown_Exposure_Complexity.pdf:**  
This work grew out of two earlier branches of our research: representation-independent pushdown lower bounds arising from completion and residual semantics, and universal higher-order compatibility fibers arising from periodic shortest-path systems. The latter showed that a large amount of higher-order information can remain completely free even after all lower-order compatibility information has been fixed. The paper identifies the physical complexity that is forced when such retained semantic information is subsequently collapsed to a common outcome. It introduces collapse-retained information as a resource and source-stack exposure as a new representation-independent measure of how deeply the original semantic representation must become physically accessible. The paper establishes a general capacity law relating retained information, representation debt, physical observation capacity, and forced exposure; determines the complete exposure spectrum with pointwise sharp bounds; and shows that universal compatibility fibers carry exact retained-information costs in every exact pushdown realization. The central novelty is a representation-independent theory of the physical cost of semantic information erasure, distinct from traditional stack height, push count, or canonical-encoding complexity. 
