...
# Exact one-work-symbol baseline: A is the single work symbol, Z is Z0.
def eta2(p,q,r):
    if r>=0:
        k,s=divmod(r,p)
        return ('P',s,k)
    u=r % q
    h=(u-r)//q
    assert h>=1 and r==u-q*h
    return ('N',u,h-1)

def raw_step2(p,q,r,ch):
    side,s,h=eta2(p,q,r)
    if side=='P':
        if ch=='0':
            if h>=1:
...
for p in range(1,31):
  for q in range(1,31):
    ...
    for r in range(-500,501):
      for ch in '01':
        got=raw_step2(p,q,r,ch)
        expected=eta2(p,q,r+(q if ch=='1' else -p))
        ck(got==expected,(p,q,r,ch,got,expected))
        onework_cases += 1
...
print('onework_cases',onework_cases)
