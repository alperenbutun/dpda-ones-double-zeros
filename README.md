...
Building on that framework, the present work introduces the $k$-fiber viewpoint and proves the **Universal $k$-Fiber Theorem**. For arbitrary integers $n \ge r \ge 2$, one fixed periodic shortest-path system admits a common exact distance and a common complete compatibility truncation through order $r-1$, while its order-$r$ layer realizes every subset of the distinguished $r$-faces—equivalently, every $r$-uniform hypergraph on $n$ vertices—and attains the maximal capacity
